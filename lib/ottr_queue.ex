defmodule Ottr do
  ...
  @impl true
  def handle_call(:fetch_task, _from, %{buffer: [], in_progress: in_progress} = state) do
    {:reply, nil, state}
  end
  ...
  def process_task(queue_name, task) do
    ...
    if :rand.uniform(10) == 1 do
      ...
      if retries < @retry_limit do
        Tasks.update_task(fetch_task, %{retries: retries, status: "pending", locked_at: nil})
        GenServer.cast(via_tuple(queue_name), {:insert, %{task | retries: retries}})
        Logger.warning("Task failed, retrying: #{inspect(task.data)} (Attempt #{retries})")
      else
        Tasks.update_task(fetch_task, %{status: "failed", locked_at: nil})
